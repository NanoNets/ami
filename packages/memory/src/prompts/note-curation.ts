 ...
 * quality contract, run daily over the notes that need it.
 */
import { WRITING_STYLE } from "@ami/shared";

export function noteCurationPrompt(): string {
  return `# Context
...
3. **Same path, same H1 title, one \`Write\` call with the complete note.**
...
The note's audience: the owner (skimming before a call) and their assistant (loading context to draft messages or prep meetings). Optimize for "everything important in the first screen."

The request message contains the Owner block (who "I" is — authoritative) and the note's current content. Rewrite the ENTIRE note with a single \`Write\` to the same path.
...
- **Source links**: keep at most the most recent "[View thread]" style link per collapsed month; keep all links in verbatim (recent) entries.
- One \`Write\` call with the complete rewritten note. Read linked notes with \`Read\` only if you must verify a link target's exact name.
