...
/** Email tag system driving note-effect filtering.
 * Triage labels each gmail signal with these tags; the graph builder skips
 * noise-tagged emails before they ever reach the note-creation agent. */
...
const DEFAULT_TAG_DEFINITIONS: TagDefinition[] = [
  ...
  { tag: "customer", type: "relationship", applicability: "both", noteEffect: "create", description: "Paying customers", example: "We're seeing great results. Can we discuss expanding to more teams?" },
  ...
  { tag: "community", type: "relationship", applicability: "both", noteEffect: "create", description: "Peers, batchmates, or open source contributors with direct interaction", example: "Love what you're building. Here's a bug I found..." },
  ...
  { tag: "infrastructure", type: "topic", applicability: "both", noteEffect: "create", description: "Deploy failures, build errors, webhook issues, API migrations, and production alerts", example: "Deploy failed for the app. Build log attached." },
  { tag: "meeting", type: "topic", applicability: "both", noteEffect: "create", description: "Calendar invites and scheduling for real meetings with named people — investors, customers, partners, candidates, team members. The key signal is a specific person you have a relationship with.", example: "Invitation: Zoom: Acme <> Nanonets sync @ Sat 7 Mar 2026" },
  ...
  { tag: "spam", type: "noise", applicability: "email", noteEffect: "skip", description: "Junk and unwanted email, including Google Groups spam moderation digests", example: "Congratulations! You've won $1,000,000..." },
  ...
  { tag: "cold-outreach", type: "noise", applicability: "email", noteEffect: "skip", description: "Unsolicited contact from someone you have no prior engagement with — includes agencies, compliance firms, content writers, dev shops, freelancers offering free work, and anyone pitching a service. Even if they mention your company by name or offer something free.", example: "We work with 230+ startups — $2000 worth of design free every month" },
  ...
  { tag: "digest", type: "noise", applicability: "email", noteEffect: "skip", description: "Community digests, forum roundups, and aggregated updates", example: "Community Weekly: 12 new posts this week..." },
  { tag: "product-update", type: "noise", applicability: "email", noteEffect: "skip", description: "Product changelogs, feature announcements, and vendor marketing disguised as tips", example: "Discover more with your free account — popular use cases inside" },
  { tag: "receipt", type: "noise", applicability: "email", noteEffect: "skip", description: "Completed transaction confirmations with no decision remaining: payment receipts, salary/payroll disbursements, tax payment acknowledgements, filing confirmations, recurring invoice-sharing threads, and transfer-initiated confirmations", example: "Challan payment under section 200 has been successfully paid." },
  ...
  { tag: "forums", type: "noise", applicability: "email", noteEffect: "skip", description: "Mailing lists, group discussions, and moderation digests that are not spam digests", example: "Re: [dev-list] Question about API design" },
  ...
  { tag: "slack", type: "source", applicability: "notes", noteEffect: "none", description: "Created or updated from a Slack message" },
  { tag: "meeting", type: "source", applicability: "notes", noteEffect: "none", description: "Created or updated from a calendar event or meeting" },
...
function tagsConfigPath(): string {
  return path.join(configDir(), "tags.json");
}

function ensureTagsConfigSync(): void {
  if (!fs.existsSync(tagsConfigPath())) {
    fs.writeFileSync(tagsConfigPath(), JSON.stringify(DEFAULT_TAG_DEFINITIONS, null, 2) + "\n", "utf8");
  }
}
...
export function getTagDefinitions(): TagDefinition[] {
  ...
  try {
    const stats = fs.statSync(tagsConfigPath());
    if (cachedTagDefinitions && cachedMtimeMs === stats.mtimeMs) return cachedTagDefinitions;
    cachedTagDefinitions = JSON.parse(fs.readFileSync(tagsConfigPath(), "utf8"));
...
/** Tags an email classifier may assign (applicability email/both). */
export function emailTagNames(): string[] {
  return getTagDefinitions()
    .filter((t) => t.applicability !== "notes")
    .map((t) => t.tag);
}

export function noiseTagNames(): Set<string> {
  return new Set(getTagDefinitions().filter((t) => t.noteEffect === "skip").map((t) => t.tag));
}
...
export function renderNoteEffectRules(): string {
  ...
  const formatList = (map: Map<string, string[]>) =>
    Array.from(map.entries()).map(([type, names]) => `- **${type}:** ${names.join(", ")}`).join("\n");
...
/** Compact tag list for the triage prompt (tag — description). */
export function renderEmailTagsForTriage(): string {
  const tags = getTagDefinitions().filter((t) => t.applicability !== "notes");
  const byType = new Map<TagType, TagDefinition[]>();
  for (const t of tags) {
  ...
  for (const [type, defs] of byType) {
    sections.push(`${TYPE_LABELS[type]}: ${defs.map((d) => d.tag).join(", ")}`);
  }
