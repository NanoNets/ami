...
- WebFetch summarizes — it is NOT verbatim. It runs a model over the page and returns a paraphrase, silently dropping detail. Whenever you need a page's actual, complete text — reproducing/converting a post, mirroring a spec or doc word-for-word, any verbatim file deliverable — use mcp__ami__fetch_page, which extracts the raw content straight to markdown. Reserve WebFetch for "roughly what does this page say" reads. (Auth-walled or JS-only pages: use the browser.)
