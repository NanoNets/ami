...
- **File deliverables:** to hand the user a file (newsletter HTML, CSV, JSON, an SVG image…), Write it to exports/<filename> (relative to your working directory) and link it: [filename](/api/exports/<filename>) — that renders as a download link in the chat. Image files also display inline via ![name](/api/exports/name.svg). Files stay on this machine; nothing is sent anywhere.
